class Day8
  class << self
    ...
    def calc(input)
      ...
      input.split("\n").each do | line|
        process_line(line)
        overall_max = [max_value, overall_max].max
      end

      puts @register_list.inspect

      puts "max at end #{max_value}"
      puts "overall max #{overall_max}"
    end

    def max_value
      max_value = @register_list.first[1]
      @register_list.each do |k, v|
    ...
    def process_line(line)
      ...
      puts "command: #{command} conditional: #{conditional}"

      command = Command.new(command)

      conditional = Conditional.new(conditional)
      if conditional.eval(@register_list)
        command.calc(register_list)
      end
...
class Conditional
  attr_accessor :register, :command, :value

  def initialize(conditional)
    conditional_elements = conditional.split(' ')
    @register = conditional_elements[0]
    @command  = conditional_elements[1]
    @value    = conditional_elements[2].to_i
  end

  def eval(register_list)
    if register_list[@register].nil?
      register_list[@register] = 0
    end
    register = register_list[@register]
...
class Command
  ...
  def initialize(command)
    ...
    @register = command_elements[0]
    @command  = command_elements[1]
    @value    = command_elements[2].to_i
  end

  def calc(register_list)
    if register_list[@register].nil?
      register_list[@register] = 0
    end

    if command == "inc"
      register_list[@register] += value
    else
      register_list[@register] -= value
    end
  end
...
xnput = "ib dec 335 if ifd > -8"

xinput = "b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"
