class Day7
  ...
  class << self
    def calc(input)
      ...
      lines.each do |line|
        elements = line.split("->")
        child_list = elements[1]

        name = elements[0].split("\s")[0]

        weight = elements[0].split("\s")[1].gsub(/\(|\)/, '')

        existing_node = node_check(root, name)

        already_added = false
        if existing_node
          already_added = true
          existing_node.weight = weight
          node = existing_node
        else
        ...
        if child_list
          names = child_list.split(",").map(&:strip)

          names.each do |name|
            temp = remove_node(root, name)
            if temp
              node.add_child(temp)
            else
              child_node = Node.new(name, 0)
      ...
      puts root.children.first[0]
    end

    # removes the node with 'name' from the node, and returns it
    def remove_node(node, name)
      if node.children.key?(name)
        node.children.each do |child_name, child|
          if child_name == name
            return node.children.delete(name)
          end
        end
      else
        node.children.each do |child_name, child|
          return remove_node(child, name)
        end
      end
    ...
    # calculates the weight of the node, but also prints how to balance the node
    def node_weight(node)
      if node.children.empty?
        return node.weight.to_i
      else
        child_sum = 0
        weight_list = {}
        node.children.each do |child_name, child|
          weight = node_weight(child)
          if weight_list[weight].nil?
        ...
        if weight_list.length > 1
          ...
          node.children.each do |child_name, child|
            if node_weight(child) == target_value
              # puts "other: #{other_value} target_val: #{target_value}"
              # puts "node val: #{child.weight.to_i}"
              diff = other_value - target_value
              puts "the first occurrence of this is the right answer"
              puts "weight should be: #{child.weight.to_i + diff}"
              puts ""
            end
          end
        end

        return child_sum + node.weight.to_i
      end
    end

    # returns node with the name specified
    def node_check(node, name)
      if node.name == name
        return node
      else
        ret_value = nil
        node.children.each do |child_name, child|
          ret_node = node_check(child, name)
          if ret_node
            ret_value = ret_node
          end
        end
    ...
    # returns the total height of the node
    def node_height(node)
      max_child_height = 0
      node.children.each do |name, child_node|
        max_child_height = [node_height(child_node), max_child_height].max
      end
      max_child_height + 1
    end

    # prints the tree in levels
    def print_tree(node)
      height = node_height(node)

      i = 1
      height.times do
        print_tree_level(node, i)
        puts ""
        i += 1
      end
    end

    # helper that determines how wide a given node should be when printing
    def space_of_children(node, initial = true)
      node_length = node.name.length + 2
      if node.children.empty?
        return node_length
      else
        children_total_length = 0
        node.children.each do |name, child_node|
          children_total_length += space_of_children(child_node, false)
        end
        if initial
          children_total_length + node.children.length - 1
        else
          children_total_length
        end
      end
    end

    # prints a tree level, takes into account how wide it should be
    DEBUG = false
    def print_tree_level(node, level)
      spaces = space_of_children(node, false)
      current_length = node.name.length + 2

      buffer = 0

      if level == 1
        return if node.name == "root"

        if current_length < spaces
          buffer = spaces - current_length
        end
        ...
        child_weights = node_weight(node)
        print " #{node.name}_#{child_weights} "
        ...
        node.children.each do |name, child_node|
          print_tree_level(child_node, level - 1)
        end
